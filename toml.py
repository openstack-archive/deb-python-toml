...
class InlineTableDict():
    """Sentinel subclass for inline tables."""
...
def loads(s, _dict=dict):
    ...
    for line in s:
        ...
        if line[0] == '[':
            ...
            ret = _load_line(line, currentlevel, multikey, multibackslash, _dict)
            if ret is not None:
...
def _load_inline_object(line, currentlevel, _dict, multikey=False, multibackslash=False):
    ...
    for group in groups:
        status = _load_line(group, currentlevel, multikey, multibackslash, _dict)
...
def _load_line(line, currentlevel, multikey, multibackslash, _dict):
    i = 1
    pair = line.split('=', i)
